...
internal static class Program
{
    ...
    private const string TaskName = "WinNetFix";
    ...
    private static int RunBackground(string configPath)
    {
        ...
        var config = Config.Load(configPath);
        var machine = new StateMachine(config, _log!);

        _log!.Info($"=== WinNetFix 启动（interval={config.Probe.IntervalSec}s, failThreshold={config.Probe.FailThreshold}）===");
    ...
    private static int ShowStatus(string configPath)
    {
        ...
        var r = probe.Probe();
        Console.WriteLine($"适配器 Up: {r.AdapterUp}  名称: {r.AdapterName ?? "-"}  网关: {r.GatewayIp ?? "-"}");
    ...
    {
        var exe = Environment.ProcessPath ?? throw new InvalidOperationException("无法获取当前程序路径");
        _log!.Info($"[Install] 注册计划任务 {TaskName}: {exe} --run");

        var tr = $"/Create /TN \"{TaskName}\" /TR \"\\\"{exe}\\\" --run\" /SC ONLOGON /RL HIGHEST /F";
        var r = ProcessRunner.Run("schtasks.exe", tr);
        if (!r.Ok)
        {
            _log.Error($"[Install] 注册失败({r.ExitCode}): {r.StdErr}");
            return 1;
        }

        Console.WriteLine("已注册开机自启计划任务 WinNetFix（登录时运行，最高权限）");
        _log.Info("[Install] 注册成功");
        return 0;
    }

    private static int UninstallTask()
    {
        _log!.Info($"[Uninstall] 注销计划任务 {TaskName}");
        var r = ProcessRunner.Run("schtasks.exe", $"/Delete /TN \"{TaskName}\" /F");
        if (!r.Ok)
        {
            _log.Error($"[Uninstall] 注销失败({r.ExitCode}): {r.StdErr}");
            return 1;
        }
        Console.WriteLine("已注销计划任务 WinNetFix");
        _log.Info("[Uninstall] 注销成功");
        return 0;
    }
